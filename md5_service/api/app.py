import os
import uuid
from fastapi import FastAPI, UploadFile, HTTPException, Depends
from ..worker.tasks import compute_md5_hash
from md5_task_data.database import get_session
from md5_task_data.crud import retrieve_md5_task
from md5_task_data.exceptions import TaskNotFound
from md5_task_data import models
from . import schemas


app = FastAPI()


@app.post("/hashes/")
async def compute_hash(file: UploadFile, session = Depends(get_session)):    # compute MD5 hash async.
    # Save file in shared directory. Consider moving to utils save_file() -> path
    filename = str(uuid.uuid4())             # unique filename
    path = os.path.join('/tmp', filename)    # or whatever shared_dir is used
    with open(path, 'wb') as data:
        data.write(await file.read())

    task = models.Md5Task()
    session.add(task)
    await session.commit()

    compute_md5_hash.delay(filename, task.task_id)   # consider deleting Md5Task in case sending fails
    return schemas.Md5TaskId(task_id=task.task_id)


@app.get("/hashes/{task_id}")
async def retrieve_hash(task_id: int, session = Depends(get_session)):
    try:
        task = await retrieve_md5_task(session, task_id)
    except TaskNotFound as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    else:
        return schemas.Md5Task.from_orm(task)

